/**
 * ProfileTimeline — Social feed style timeline
 * 
 * Pixel-perfect replica of the Vristo "Profile" timeline variant.
 * Each item shows: avatar with connector line, name, time, content with icon+title, 
 * description, and optional image thumbnails (avatar or grid).
 */
import React from 'react'

export default function ProfileTimeline({ items, compact }) {
    return (
        <div className="mb-5">
            {items.map((group, gi) => (
                <div key={gi}>
                    {group.label && (
                        <p className="mb-5 text-base font-bold text-white-dark">{group.label}</p>
                    )}
                    {(group.entries || [group]).map((item, idx) => (
                        <div className="sm:flex" key={item.id || idx}>
                            {/* Avatar column with connector line */}
                            <div
                                className={`relative z-[2] mx-auto mb-5 before:absolute before:top-12 before:left-1/2 before:-bottom-[15px] before:-z-[1] before:hidden before:h-auto before:w-0 before:-translate-x-1/2 before:border-l-2 before:border-[#ebedf2] dark:before:border-[#191e3a] sm:mb-0 sm:before:block ltr:sm:mr-8 rtl:sm:ml-8`}
                            >
                                {item.avatar ? (
                                    <img
                                        src={item.avatar}
                                    ...
                                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary font-bold shadow-[0_4px_9px_0_rgba(31,45,61,0.31)]">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="mt-4 mb-16 sm:mt-7">
                                    {/* Icon + Title */}
                                    {item.icon && (
                                        <span className="inline-block h-5 w-5 align-text-bottom text-white-dark ltr:mr-2.5 rtl:ml-2.5">
                                    ...
                                    <h6 className="mb-2 inline-block text-lg font-bold">{item.title}</h6>
                                    ...
                                    {/* Thumbnails — avatars row */}
                                    {item.thumbnails && item.thumbnailType === 'avatar' && (
                                        <div className="mt-6 flex space-x-1 ltr:pl-8 rtl:space-x-reverse rtl:pr-8">
                                            {item.thumbnails.map((thumb, ti) => (
                                                <img
                                                    key={ti}
                                                    src={thumb}
                                                    alt="image"
                                    ...
                                    {item.thumbnails && item.thumbnailType === 'grid' && (
