 ...
 * Supports mouse-drag resizing (min 229px, max 500px)
 */
...
export default function RecordsSidebar({
    ...
    const currentWidthRef = useRef(externalWidth || SIDEBAR_DEFAULT)
    const onWidthChangeRef = useRef(onSidebarWidthChange)

    // Keep refs in sync
    useEffect(() => { onWidthChangeRef.current = onSidebarWidthChange }, [onSidebarWidthChange])
    useEffect(() => { currentWidthRef.current = localWidth }, [localWidth])

    // Sync external width changes
    useEffect(() => {
        if (externalWidth && !isDraggingRef.current) {
            setLocalWidth(externalWidth)
        }
    }, [externalWidth])
    ...
    useEffect(() => {
        ...
        const handleMouseUp = () => {
            ...
            // Persist width to server (read from ref to avoid stale closure)
            if (onWidthChangeRef.current) {
                onWidthChangeRef.current(currentWidthRef.current)
            }
        }
    ...
    }, []) // Only mount once — refs provide fresh values
    ...
    return (
        <div style={{ position: 'relative', width: localWidth, minWidth: SIDEBAR_MIN, maxWidth: SIDEBAR_MAX, flexShrink: 0 }}>
            <div className="panel z-10 space-y-4 overflow-y-auto p-4 h-full"
            ...
            <div
                ...
                onMouseEnter={(e) => { e.currentTarget.querySelector('.resize-bar').style.opacity = '1' }}
                onMouseLeave={(e) => { if (!isDraggingRef.current) e.currentTarget.querySelector('.resize-bar').style.opacity = '0' }}
            >
