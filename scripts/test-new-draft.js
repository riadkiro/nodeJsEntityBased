...
async function run() {
    try {
        ...
        const templateId = '6a0cc7098582f628068fb6d7'; // Nouveau modèle template
        ...
        // Inject additional records
        const additionalRecordsLoaded = [];
        for (const addId of additionalRecordIds) {
        ...
        console.log('--- GENERATING RESOLVED CONTENT FOR NEW DRAFT ---');
        const docTemplate = await Document.findById(smartDocTemplate.documentId).lean();
        const testTemplateStr = `test 65656565\n\n{{entreprises.title}}\n{{contacts.title}}\n{{opportunites.title}}`;
